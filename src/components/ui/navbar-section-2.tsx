...
type NavItem = {
  label: string;
  href?: string;
  panelId?: "product" | "solution" | "pricing" | "about" | "technology" | "impact";
};
...
function PanelCard({ item, compact = false, onNavigate }: { item: ProductCard; compact?: boolean; onNavigate?: (href: string) => void }) {
  ...
  return (
    <a href={item.href || "#"} onClick={handleClick} className={compact ? "group relative flex h-[86px] flex-col justify-center overflow-hidden rounded-xl border border-zinc-900 bg-zinc-950/40 p-3 transition-all hover:border-zinc-800 hover:bg-zinc-900/15" : "group relative flex min-h-[132px] flex-col justify-between overflow-hidden rounded-xl border border-zinc-900 bg-zinc-950/40 p-4 transition-all hover:border-zinc-800 hover:bg-zinc-900/15"}>
      ...
      <div className="relative z-10">
        ...
        <p className="mt-1 text-[10px] leading-normal text-zinc-400 transition-colors group-hover:text-zinc-300">{item.description}</p>
      </div>
      {!compact && <span className="relative z-10 mt-5 text-[10px] font-medium text-amber-400">Explore <ArrowRight className="ml-1 inline size-3" /></span>}
    </a>
  );
}

function DropdownPanel({ panel, onNavigate }: { panel: MenuPanel; onNavigate?: (href: string) => void }) {
  if (panel.id === "product") {
    const [chat, build, imagine, voice] = panel.items;
    return (
      <div className="grid grid-cols-[1.2fr_1.2fr_1fr] gap-4 bg-black p-4">
        <PanelCard item={chat} onNavigate={onNavigate} />
        <PanelCard item={build} onNavigate={onNavigate} />
        <div className="flex flex-col gap-2"><PanelCard item={imagine} compact onNavigate={onNavigate} /><PanelCard item={voice} compact onNavigate={onNavigate} /></div>
      </div>
    );
  }

  return <div className="grid grid-cols-3 gap-4 bg-black p-4">{panel.items.map((item) => <PanelCard key={item.title} item={item} onNavigate={onNavigate} />)}</div>;
}
...
/* NEPeD Tailored Navbar Component utilizing the NavbarSectionTwo dropdown panel system */
const nepedNavItems: NavItem[] = [
  { label: "About", panelId: "about" },
  { label: "Agroforestry Heritage", href: "/neped-economic" },
  { label: "Technology", panelId: "technology" },
  { label: "Impact", panelId: "impact" },
  { label: "Projects", href: "/#projects" },
  { label: "Blog", href: "/blog" },
];

const nepedMenuPanels: MenuPanel[] = [
  {
    id: "about",
    items: [
      { title: "Overview & Vision", description: "Our history, registered society mandate, and core energy objectives.", href: "/about#overview" },
      { title: "Our Team", description: "Meet the inception and present team members driving innovation.", href: "/about#team" },
    ],
  },
  {
    id: "technology",
  ...
  },
  {
    id: "impact",
    items: [
      { title: "Social Benefits", description: "Empowering rural youth, women, and farmers with grid-free power.", href: "/impact#benefits" },
      { title: "Deployments Directory", description: "Sustainable energy projects implemented across Nagaland & NE India.", href: "/impact#deployments" },
    ],
  },
];

export function NepedNavbarSection() {
  const [activeMenu, setActiveMenu] = useState<string | null>(null);
  const [mobileOpen, setMobileOpen] = useState(false);
  const [mobileActiveMenu, setMobileActiveMenu] = useState<string | null>(null);
  const navigate = useNavigate();

  const activePanel = nepedMenuPanels.find((panel) => panel.id === activeMenu);
  ...
  return (
    <motion.div
      ...
      className="sticky top-4 sm:top-6 z-50 w-full max-w-[1200px] mx-auto mb-6 select-none font-sans text-ink"
    >
      <div className="relative z-30 hidden h-14 w-full items-center justify-between lg:flex">
        {/* NEPED Brand Logo */}
        <Link
          to="/"
        ...
        {/* Center Floating Dark Dropdown Panel */}
        <div className="absolute left-1/2 top-0 hidden w-[680px] -translate-x-1/2 lg:block" style={{ filter: "drop-shadow(0 12px 24px rgba(0, 0, 0, 0.15))" }}>
          <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg" className="pointer-events-none absolute -left-[18px] top-0 z-10 text-black"><path d="M 20 20 L 20 0 L 0 0 C 11.046 0 20 11.046 20 20 Z" fill="currentColor" /></svg>
          <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg" className="pointer-events-none absolute -right-[18px] top-0 z-10 text-black"><path d="M 0 0 L 20 0 C 8.954 0 0 8.954 0 20 Z" fill="currentColor" /></svg>

          <motion.div animate={{ height: activePanel ? 230 : 48 }} transition={{ type: "spring", stiffness: 300, damping: 28 }} className="relative flex w-full flex-col justify-start overflow-hidden bg-black" style={{ borderBottomLeftRadius: "16px", borderBottomRightRadius: "16px" }}>
            <div className="z-20 flex h-12 items-center justify-center px-6">
              <nav className="flex w-full items-center justify-center gap-6 text-sm font-medium text-zinc-400">
                {nepedNavItems.map((item) => item.panelId ? (
                  ...
                  <button key={item.label} onClick={() => handleNavigation(item.href || "/")} className="px-2 py-1 text-zinc-400 transition-colors duration-250 hover:text-white cursor-pointer">
                    {item.label}
                  </button>
                ))}
              </nav>
            ...
            <AnimatePresence mode="wait">
              {activePanel && (
                <motion.div key={activePanel.id} initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.18 }} className="overflow-hidden border-t border-zinc-900 bg-black">
                  <DropdownPanel panel={activePanel} onNavigate={handleNavigation} />
                </motion.div>
              )}
            </AnimatePresence>
          </motion.div>
